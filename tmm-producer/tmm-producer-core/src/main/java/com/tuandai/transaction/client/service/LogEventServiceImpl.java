...
import com.tuandai.transaction.client.utils.*;
...
public class LogEventServiceImpl implements LogEventService {
    ...
    public LogEventServiceImpl() {
        ...
        errorLogFile = new File(SettingSupport.getRpcDirCanonicalPath() + File.separatorChar + ConstantUtils.errorLogFile);
    ...
    private List<EventDefinition> doReadLogEvent(Integer size) {
        ...
        CacheMapFileUtils.readDataToMap(checkPoint.checkFile, new CacheMapFileUtils.ReadDataToMapProcess() {
            ...
            protected boolean process(String keyValueStr) throws Exception {
                if (keyValueStr.equals(RPC_FILE_END_MARK)) {
                    ...
                    try {
                        event = JSONObject.parseObject(keyValueStr, EventDefinition.class);
                    } catch (Exception e) { // 序列化发送错误的是时候，将数据存储到 err.map
                        logger.warn("tmm 反序列化日志发生未知异常，请检查数据是否正常，跳过该日志记录，并保存到 err.map");
                        errorLogs.add(keyValueStr);
                    }
                    if (list.size() >= size) {
                        return false;
                    }
        ...
        if (!CollectionUtils.isEmpty(errorLogs)) {
            // 保存 err.map 日志
            CacheMapFileUtils.writeDataToFile(errorLogFile, true, new CacheMapFileUtils.WriteDataToFileProcess() {
                @Override
                protected void process(OutputStreamWriter writer) throws Exception {
                    // 写日志文件
