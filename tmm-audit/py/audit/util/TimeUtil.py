...
def get_ms_back_hours(hours,ms=int(1000 * time.time())):
    '''
    :param ms: 传入毫秒时间戳
    :param hours: 传入后退小时数
    :return 返回后退小时数后的时间戳
...
def get_utc_string_to_local_ts(utc_time_str, utc_format='%Y-%m-%dT%H:%M:%S.%f'):
    '''
    UTCS时间串转换为当前时区时间戳 
...
if __name__ == '__main__':
    ...
    print '%.0f' % get_ms_back_hours(1)
    print get_utc_string_to_local_ts("2018-11-20T02:00:43.186")
