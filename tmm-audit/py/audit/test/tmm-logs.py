...
def es_put(_index,_type,_id,values):
    ...
    val={}
    val["message"]=jdata
    val["addDate"]=int(round(time.time() * 1000))
    http_put(url,val)
...
def start_tmm(sec):
    while True:
        ...
        val={}
        val={'type':'tmmProduce','producer':'producer-service-name1','ptime':ptime,'uid':uid ,'st':1}
        es_put("log4j-20181122","log4j",_id,val)

        _id=str(int(round(time.time() * 1000))*10 + ri)
        ctime=ptime + random.randint(100,10000)
        val.clear()
        val={'type':'tmmConsume','producer':'producer-service-name1','ptime':ptime,'ctime':ctime ,'uid':uid ,'consumer':'consumer-service-name1'}
        es_put("log4j-20181122","log4j",_id,val)

        _id=str(int(round(time.time() * 1000))*10 + ri)
        ctime=ptime + random.randint(100,10000)
        val.clear()
        val={'type':'tmmConsume','producer':'producer-service-name1','ptime':ptime,'ctime':ctime ,'uid':uid ,'consumer':'consumer-service-name2'}
        if ctime % 11 != 0 :
            es_put("log4j-20181122","log4j",_id,val)
